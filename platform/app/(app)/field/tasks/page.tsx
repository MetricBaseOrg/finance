...
function FieldTasksInner() {
  ...
  async function create(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setForm({ title: '', assigneeId: '', dueDate: '', priority: 'MEDIUM', refKind: 'none', refEntityId: '' })
      loadTasks()
    } else {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <Panel title="Assign task" sub="Assign field work to a teammate. It's tracked as a task in ProBase — board, due dates, and notifications included.">
        <form onSubmit={create} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, alignItems: 'end' }}>
          ...
          <label style={{ display: 'grid', gap: 5 }}><WsLabel>Due</WsLabel><input type="date" style={wsField} value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} /></label>
      ...
      <Panel title="Field tasks" sub="Assigned field work. Open in ProBase to manage status, comments, and subtasks." pad={0}
        ...
        {loading ? <div style={{ padding: 18, color: 'var(--mb-ink-muted)', fontSize: 12.5 }}>Loading…</div> : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>{['Task', 'Assignee', 'Linked', 'Priority', 'Due', 'Status'].map((h) => <th key={h} style={{ textAlign: 'left', fontSize: 10, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--mb-ink-soft)', padding: '10px 16px', fontFamily: 'var(--mb-font-mono)' }}>{h}</th>)}</tr></thead>
            <tbody>
              ...
              {tasks.map((t) => (
                <tr key={t.id} style={{ borderTop: '1px solid var(--mb-divider)' }}>
                  ...
                  <td className="mb-num" style={{ padding: '10px 16px', fontSize: 11.5, color: 'var(--mb-ink-muted)' }}>{t.dueDate ? new Date(t.dueDate).toISOString().slice(0, 10) : '—'}</td>
