...
const TYPES = ["BANK", "CASH", "CRYPTO", "BROKERAGE", "CREDIT", "PROJECT", "OTHER"] as const;
...
export function AccountRow({
  ...
  const projectName = account.projectId ? projects.find((p) => p.id === account.projectId)?.name : null;
  ...
  if (editing) {
    return (
      <>
        {/* Desktop edit row */}
        <form onSubmit={handleUpdate} className="hidden md:flex flex-col gap-2 px-4 py-2 border-b border-line bg-[var(--color-bg-hover)]">
          <div className="grid grid-cols-[1fr_120px_120px_160px_120px] items-end gap-2">
            <input name="name" defaultValue={account.name} required maxLength={60} className="mb-input text-sm py-1.5" />
            <select name="type" value={type} onChange={(e) => setType(e.target.value)} className="mb-input text-sm py-1.5">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setEditing(false)} className="font-mono rounded-lg text-[10px] uppercase tracking-[0.18em] text-gray-3 hover:text-gold transition-colors">
                Cancel
              </button>
            </div>
          </div>
          {type === "PROJECT" && (
            <select name="projectId" defaultValue={account.projectId ?? ""} required className="mb-input text-sm py-1.5 max-w-xs">
        ...
        <form onSubmit={handleUpdate} className="md:hidden px-4 py-4 border-b border-line bg-[var(--color-bg-hover)] flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="flex flex-col gap-1">
              ...
              <select name="type" value={type} onChange={(e) => setType(e.target.value)} className="mb-input">
                {TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </label>
            <label className="flex flex-col gap-1">
          ...
          {type === "PROJECT" && (
            <label className="flex flex-col gap-1">
              <span className="label-mono">Linked project</span>
              <select name="projectId" defaultValue={account.projectId ?? ""} required className="mb-input">
                <option value="" disabled>Select a project…</option>
                {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </label>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => setEditing(false)} className="font-mono rounded-lg text-[11px] uppercase tracking-[0.18em] text-gray-3 hover:text-gold transition-colors">Cancel</button>
          </div>
        </form>
      </>
    );
  ...
  return (
    <div className={`border-b border-line last:border-b-0 transition-colors ${archived ? "" : "hover:bg-[var(--color-bg-hover)]"}`}>
      ...
      <div className="hidden md:grid grid-cols-[1fr_120px_120px_160px_120px] px-4 py-3 items-center">
        <span className={`font-sans ${archived ? "text-gray-2" : "text-white"}`}>
          {account.name}
          {projectName && <span className="ml-2 font-mono text-[10px] text-gold">· {projectName}</span>}
        </span>
        ...
        {canManage && (
          <div className="flex gap-3 justify-end">
            {!archived && (
              <button type="button" onClick={() => setEditing(true)} className="font-mono rounded-lg text-[10px] uppercase tracking-[0.2em] text-gray-3 hover:text-gold transition-colors">
                Edit
              </button>
            )}
            <button type="button" onClick={handleArchive} disabled={pending} className="font-mono rounded-lg text-[10px] uppercase tracking-[0.2em] text-gray-3 hover:text-gold transition-colors disabled:opacity-50">
      ...
      <div className="md:hidden px-4 py-4 flex flex-col gap-2">
        <div className="flex justify-between items-start gap-3">
          <div className="flex flex-col min-w-0">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-gray-3 mt-1">
              ...
              {projectName && <span className="text-gold"> · {projectName}</span>}
            </span>
          </div>
        ...
        {canManage && (
          <div className="flex gap-4 justify-end pt-1">
            {!archived && (
              <button type="button" onClick={() => setEditing(true)} className="font-mono rounded-lg text-[10px] uppercase tracking-[0.2em] text-gray-3 hover:text-gold transition-colors">Edit</button>
            )}
            <button type="button" onClick={handleArchive} disabled={pending} className="font-mono rounded-lg text-[10px] uppercase tracking-[0.2em] text-gray-3 hover:text-gold transition-colors disabled:opacity-50">
              {pending ? "…" : archived ? "Unarchive" : "Archive"}
            </button>
