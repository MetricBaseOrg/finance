...
// Finance audit vocabulary (was the AuditAction / AuditEntityType enums before
// the platform unified onto a generic AuditLog). Kept as static lists to drive
// the filter dropdowns.
const ACTIONS = [
  "CREATE", "UPDATE", "DELETE", "ARCHIVE", "UNARCHIVE",
  "INVITE_CREATE", "INVITE_REVOKE", "INVITE_ACCEPT",
  "MEMBER_ROLE_CHANGE", "MEMBER_REMOVE",
  "INVESTMENT_BUY", "INVESTMENT_SELL", "INVESTMENT_DIVIDEND",
];
const ENTITIES = [
  "ACCOUNT", "TRANSACTION", "CATEGORY", "WORKSPACE", "FX_RATE",
  "MEMBERSHIP", "INVITE", "INVESTMENT_POSITION", "INVESTMENT_LOT", "DIVIDEND",
];

function summaryOf(metadata: string | null): string {
  if (!metadata) return "";
  try {
    const parsed = JSON.parse(metadata) as { summary?: string };
...
export default async function ActivityPage({
  ...
  if (sp.action && ACTIONS.includes(sp.action)) where.action = sp.action;
  if (sp.entity && ENTITIES.includes(sp.entity)) where.entity = sp.entity;
  if (sp.member) where.actorId = sp.member;
  if (sp.from || sp.to) {
  ...
  return (
    <div className="flex flex-col gap-8 max-w-[1240px]">
      ...
      <AuditFilters
        actions={ACTIONS}
        entities={ENTITIES}
        members={members.map((m) => ({ id: m.userId, email: m.user.email }))}
      />
            ...
            >
              ...
              <div className="md:hidden px-4 py-4 flex flex-col gap-1">
                ...
                <div className="flex items-center gap-1.5 font-mono text-[10px] text-gray-3 uppercase tracking-[0.18em]">
                  ...
                  <span className="truncate">{l.actor?.email ?? "system"}</span>
                </div>
              </div>
            </div>
          ))}
...
function PageLink({
  ...
  if (disabled) {
    return (
      <span className="font-mono rounded-lg text-[11px] uppercase tracking-[0.18em] text-gray-4 px-3 py-1.5 border border-line">
        {children}
      </span>
    );
  }
  ...
  return (
    <Link
      href={`/finance/${slug}/settings/activity?${qp.toString()}`}
      className="font-mono rounded-lg text-[11px] uppercase tracking-[0.18em] text-gold hover:text-gold-bright px-3 py-1.5 border border-line hover:border-gold transition-colors"
    >
      {children}
    </Link>
  );
