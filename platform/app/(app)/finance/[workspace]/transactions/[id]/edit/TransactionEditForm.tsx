...
export function TransactionEditForm({
  ...
  const [state, formAction, pending] = useActionState<TxnActionState, FormData>(
    async (prev, fd) => {
      ...
      if (!next?.error) {
        router.push(`/finance/${slug}/transactions`);
      }
      return next;
    },
    {},
  ...
  return (
    <form action={formAction} className="mb-card p-6 flex flex-col gap-5">
      ...
      <div className="flex gap-3">
        ...
        <button
          type="button"
          onClick={() => router.push(`/finance/${slug}/transactions`)}
          className="font-mono rounded-lg text-[11px] uppercase tracking-[0.18em] text-gray-2 hover:text-gold transition-colors px-4 py-2.5 border border-line hover:border-gold"
        >
          Cancel
        </button>
      </div>
