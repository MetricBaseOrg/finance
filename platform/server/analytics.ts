...
async function recentTxns(organizationId: string) {
...
export async function buildDashboard(
  ...
  const incomeAgg = await db.transaction.groupBy({
    by: ["categoryId"],
    where: { organizationId, date: { gte: monthStart, lt: nextMonthStart }, type: "INCOME" },
    _sum: { baseAmount: true },
  });
  ...
  const recent = await recentTxns(organizationId);
  ...
  const accounts = await db.finAccount.findMany({ where: { organizationId }, select: { openingBalance: true } });
  let totalOpening = 0;
  ...
  const [priorInc, priorExp] = await Promise.all([
    db.transaction.aggregate({ where: { organizationId, type: "INCOME", date: { lt: seriesStartDate } }, _sum: { baseAmount: true } }),
    db.transaction.aggregate({ where: { organizationId, type: "EXPENSE", date: { lt: seriesStartDate } }, _sum: { baseAmount: true } })
  ]);
  ...
  const yearTxns = await db.transaction.findMany({
    where: { organizationId, type: { in: ["INCOME", "EXPENSE"] }, date: { gte: seriesStartDate } },
    select: { date: true, type: true, baseAmount: true },
    orderBy: { date: "asc" }
  });
...
export async function buildBudgets(organizationId: string): Promise<BudgetRow[]> {
  const now = new Date();
...
export async function buildAlerts(organizationId: string): Promise<BudgetAlert[]> {
  const rows = await buildBudgets(organizationId);
  return rows
    .filter(
      (r) =>
        r.kind === "EXPENSE" &&
