...
export async function createInvite(
  ...
  revalidatePath(`/finance/settings/members`);
...
export async function revokeInvite(slug: string, inviteId: string) {
  ...
  const invite = await db.orgInvite.findFirst({
    where: { id: inviteId, organizationId: workspace.id },
  });
  if (!invite) return;
...
export async function changeMemberRole(
  ...
  const membership = await db.membership.findFirst({
    where: { id: parsed.data.membershipId, organizationId: workspace.id },
    include: { user: true },
  });
...
export async function removeMember(
  ...
  const { user, workspace } = await requireRole(slug, ["OWNER", "ADMIN"]);
  const membership = await db.membership.findFirst({
    where: { id: membershipId, organizationId: workspace.id },
    include: { user: true },
  });
  if (!membership) return { error: "Member not found." };
